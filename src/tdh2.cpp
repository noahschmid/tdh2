...
namespace Botan {
	...
	uint8_t* TDH2_PublicKey::extract_label(std::vector<uint8_t> encryption) {
		if (encryption.size() < 4*m_group.p_bytes() + 21) {
			throw Invalid_Argument("encryption too short");
		}

		uint8_t label[20];
		std::copy(encryption.end() - (4*m_group.p_bytes() + 20), encryption.end() - 4*m_group.p_bytes(), label);
	...
	std::vector<uint8_t> TDH2_PublicKey::encrypt(std::vector<uint8_t> msg, uint8_t label[20], RandomNumberGenerator& rng) {
		...
		BigInt f = m_group.mod_p(s + m_group.multiply_mod_p(r, e));
		...
		std::cout << "out size after: " << out.size() << "\n";
		std::cout << "p_bytes: " << m_group.p_bytes() << "\n";
	...
	bool TDH2_PublicKey::verify_share(std::vector<uint8_t> share, std::vector<uint8_t> encryption) {
		...
		BigInt w = m_group.power_g_p(f) / power_mod(u, e, m_group.get_p());
		BigInt w_hat = power_mod(get_g_hat(), f, m_group.get_p()) / power_mod(u_hat, e, m_group.get_p());

		size_t id = encryption.at(0);
		size_t valid = encryption.at(1);
		BigInt hi = m_h.at(id);
		...
		if (e == h2(c, l, u, w, u_hat, w_hat, m_group.get_q()) && share.size() == 2 + 3*m_group.p_bytes()) {
			...
			BigInt hi_hat = m_group.power_g_p(fi) / power_mod(hi, ei, m_group.get_p());
			BigInt ui_hat = power_mod(u, fi, m_group.get_p()) / power_mod(ui, ei, m_group.get_p());
	...
	std::vector<TDH2_PartialPrivateKey> TDH2_PartialPrivateKey::generate_keys(uint8_t k, 
		...
		BigInt g_hat = group.power_g_p(x, group.p_bits());
		...
		for(uint8_t i = 1; i != n + 1; ++i) {
			...
			h.push_back(group.power_g_p(xi.at(i-1)));
		}
	...
	BigInt get_lag_coeff(std::vector<uint8_t> ids, BigInt i, BigInt q) {
		BigInt res = 1;

		for(BigInt j : ids) {
			if (i != j)
				res *= ((j % q) * inverse_mod((j - i)%q, q)) % q;
		}
		return res % q;
	}
	...
	std::vector<uint8_t> TDH2_PartialPrivateKey::decrypt_share(std::vector<uint8_t> encryption, RandomNumberGenerator &rng) {
		...
		if (e == h2(c, l, u, w, u_hat, w_hat, m_group.get_q())) {
			valid = 1;
			...
			BigInt hi_hat = m_group.power_g_p(si);
		...
		std::cout << "share: " << hex_encode(share) << "\n";
	...
	std::vector<uint8_t> TDH2_PartialPrivateKey::combine_shares(std::vector<uint8_t> encryption, std::vector<std::vector<uint8_t>> shares) {
		...
		for (int i = 0; i != shares.size(); ++i) {
			BigInt l = get_lag_coeff(ids, shares.at(i).at(0), m_group.get_q());
			BigInt ui = BigInt::decode(shares.at(i).data() + 2, m_group.p_bytes());
		...
		std::cout << "\nreconstructed rG: " << rG << std::endl;
		std::cout << encryption.size() << std::endl;
		std::cout << (4 * get_group().p_bytes() + 20) << "\n";
	...
	std::vector<uint8_t> TDH2_PartialPrivateKey::reconstruct_secret(std::vector<TDH2_PartialPrivateKey> keys) {
		...
		for(uint8_t i = 0; i != keys.size(); ++i) {
			key += get_lag_coeff(ids, keys.at(i).get_id(), q) * keys.at(i).get_xi();
		}
